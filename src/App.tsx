...
import { Book, Heart, History, MessageCircle, Settings, Trophy, Sparkles, Loader2, Music, Play, Pause, Volume2, Wind, Plus, Trash2, Check, ChevronDown, Users, ThumbsUp, Globe, Leaf, Brain, Target, Share2, X, LogOut } from 'lucide-react';
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'fish' | 'scripture' | 'assistant' | 'history' | 'meditation' | 'settings' | 'community' | 'vow' | 'merit' | 'dashboard'>('fish');
  ...
  const [isGuest, setIsGuest] = useState(identityService.isGuest());
  const [userId, setUserId] = useState(identityService.getUserId());
  ...
  const finishSession = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `我刚刚完成了一次修行。念诵内容是：${activeScripture.title}，念诵次数：${sessionCount}次，时长：${Math.floor(duration / 60)}分${duration % 60}秒。请作为禅师给我一段简短的、充满智慧的“回想”或感悟，字数在50字以内。`,
      });
      ...
      <header className="sticky top-0 z-[60] bg-zen-bg/90 backdrop-blur-md border-b border-zen-accent/10">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 rounded-full bg-zen-accent flex items-center justify-center text-white shrink-0 mt-1">
                <Heart className="w-6 h-6 fill-current" />
              </div>
              <div className="text-left flex-1 min-w-0">
                <p className="text-lg font-serif font-bold text-zen-ink">禅心助手</p>
                <p className="text-sm text-zen-ink/70 font-medium mt-0.5 mb-1.5">
                  {(() => {
                ...
                <div className="flex items-center gap-3">
                  <p className="text-xs text-purple-600/80 truncate max-w-[200px] sm:max-w-[300px]" title={personalVow || t('default_vow_text')}>
            ...
            <div className="flex items-center gap-3 sm:gap-6">
              <div className="flex items-center gap-1.5 sm:gap-2 mr-1 sm:mr-2">
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 w-8 h-8 sm:w-auto sm:h-auto sm:px-3 sm:py-1.5 bg-white/50 hover:bg-white/80 rounded-full text-xs font-medium text-zen-ink/70 transition-colors border border-zen-accent/10"
                  title={isGuest ? '游客登录' : '已登录'}
                >
                  <Users className="w-4 h-4 sm:w-3.5 sm:h-3.5" />
                  <span className="hidden sm:inline">{isGuest ? '游客登录' : '已登录'}</span>
                </button>
                <button
              ...
              <div className="flex flex-col items-end">
                <p className="text-[10px] uppercase tracking-widest text-zen-accent/50 font-bold">连续修行</p>
                ...
                <ThoughtCollector className="mt-1.5 !px-3 !py-1.5 !text-xs" />
              </div>
            </div>
        </div>
      </header>
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'fish' && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              className="flex flex-col items-center justify-center py-6 space-y-8 min-h-[50vh]"
            >
              <div className="w-full max-w-md space-y-8">
                ...
                <div className="bg-white p-6 rounded-3xl border border-zen-accent/5 shadow-sm">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full appearance-none bg-zen-bg/50 border border-zen-accent/10 rounded-xl px-4 py-3 text-sm font-medium text-zen-accent focus:outline-none focus:border-zen-accent/30 pr-10"
                    >
                  ...
                  {!isSessionActive && (
                    <div className="flex justify-center mt-4">
                      <button 
                        ...
                        className="w-full bg-zen-accent text-white py-3 rounded-xl font-bold shadow-lg hover:scale-[1.02] transition-transform flex items-center justify-center gap-2"
                      >
                ...
                {activeScripture.category !== 'name' && (
                  <div className="bg-white p-6 rounded-3xl border border-zen-accent/5 shadow-sm max-h-96 overflow-y-auto">
                    <h3 className="text-center font-serif font-bold text-zen-accent mb-4">{activeScripture.title}</h3>
                    <p className="text-zen-ink/80 font-serif leading-relaxed whitespace-pre-wrap text-center text-sm">
                      {activeScripture.content}
                    </p>
                  </div>
                )}

                {/* Observe Thoughts Button */}
                  ...
                  <div className="flex justify-center">
                    <div className="flex flex-col items-center gap-4">
                      <div className="flex items-center gap-8 bg-white px-8 py-4 rounded-full border border-zen-accent/10 shadow-sm">
                        <div className="text-center">
                          <p className="text-[10px] uppercase tracking-widest text-zen-accent/40 font-bold">{t('session_merit')}</p>
                          <p className="text-2xl font-serif font-bold text-zen-accent">
                      ...
                      <p className="text-xs text-zen-accent/40 animate-pulse italic">{t('practicing')}</p>
                    </div>
                  </div>
                )}

                {/* Wooden Fish */}
                <div className="scale-75 origin-center -mt-16 -mb-8">
                  <WoodenFish 
                    onHit={handleHit} 
          ...
          {activeTab === 'meditation' && (
            ...
            >
              <Meditation onFinish={(session) => {
                ...
                setHistory(prev => [newSession, ...prev]);
                localStorage.setItem('zen_history', JSON.stringify([newSession, ...history]));
              }} />
      ...
      <nav className="fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-md border-t border-zen-accent/10 pb-safe pt-2 px-4 z-50">
        <div className="flex justify-around items-center max-w-2xl mx-auto">
          {[
            ...
            { id: 'scripture', icon: Book, label: t('tab_scripture') },
            { id: 'merit', icon: Globe, label: t('tab_merit') },
