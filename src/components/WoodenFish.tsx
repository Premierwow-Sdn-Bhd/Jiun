      ...
      <AnimatePresence>
        {floatingTexts.map(text => (
          <motion.div
            ...
            animate={{ opacity: 0, y: -100, scale: text.isCombo ? 1.5 : 1.2 }}
            exit={{ opacity: 0 }}
            className={cn(
              "absolute font-medium pointer-events-none whitespace-nowrap z-10",
              text.isCombo ? "text-amber-500 font-bold drop-shadow-md" : "text-zen-accent"
            )}
            style={{ left: `calc(50% + ${text.x}px)`, top: `-40px` }}
          >
