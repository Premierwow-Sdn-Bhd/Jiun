...
export const AuthModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  const [hasAgreed, setHasAgreed] = useState(() => {
    return localStorage.getItem('zen_agreed_to_terms') === 'true';
  });
  const [agreedToTerms, setAgreedToTerms] = useState(hasAgreed);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (data.mock) {
        // Mock login directly without opening a popup
        identityService.setUserId('google_user_' + Math.random().toString(36).substr(2, 9));
      ...
      if (!authWindow) {
        setError(t('auth_error_popup'));
        setIsLoading(false);
      } else {
        // We don't set isLoading(false) here because we are waiting for the popup message.
        // But we should set a timeout to reset it in case the user closes the popup without logging in.
