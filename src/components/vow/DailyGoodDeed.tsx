...
import { Sparkles, Loader2, Send, ShieldCheck, AlertCircle, Calendar, ChevronDown, ChevronUp } from 'lucide-react';
...
export const DailyGoodDeed = ({ user, onLevelUp }: { user: any, onLevelUp: (msg: string) => void }) => {
  ...
  return (
    <div className="bg-white rounded-[40px] p-8 shadow-sm border border-zen-accent/5 space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-serif font-bold text-zen-ink">每日一善</h2>
        <button 
          onClick={() => setIsRepentance(!isRepentance)}
          className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold ${isRepentance ? 'bg-red-100 text-red-600' : 'bg-zen-bg text-zen-accent'}`}
        >
          {isRepentance ? <ShieldCheck className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />}
          ...
          <div className="flex flex-wrap gap-2">
            {(isRepentance 
              ...
              <button
                ...
                className="px-3 py-1 bg-zen-accent/5 text-zen-accent rounded-full text-xs hover:bg-zen-accent/10 transition-colors"
              >
                + {tag}
              </button>
            ))}
