...
const EMOTION_GROUPS = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'wronged', label: '委屈', color: 'text-purple-600', bg: 'bg-purple-600/10' },
...
const ALL_EMOTIONS = EMOTION_GROUPS.flatMap(g => g.options);

export const ThoughtCollector = ({ className }: { className?: string }) => {
  ...
  const uniqueYears = Array.from(new Set([new Date().getFullYear().toString(), ...thoughts.map(t => new Date(t.timestamp).getFullYear().toString())])).sort((a, b) => b.localeCompare(a));
  ...
  useEffect(() => {
    const saveThoughts = async () => {
      ...
      // Trigger sync
      syncService.syncLogs(encryptedThoughts).then((result) => {
        if (result && result.server_updates && result.server_updates.length > 0) {
          // In a real app, we would merge these updates back into local state
          console.log('Server updates received:', result.server_updates);
        }
      });
    };
  ...
  const handleGetGuidance = async (thought: Thought) => {
    ...
    try {
      ...
      const prompt = `你是一位慈悲的导师以5戒8正道4圣谛和正道的佛法来说法。用户感受到了 ${emoLabels}，因为 ${thought.event}。他尝试用 ${thought.action} 转念。请给予 100 字以内的睿智点评。`;
  ...
  return (
    <>
      ...
      <motion.button
        ...
        className={cn(
          "px-3 py-1.5 bg-zen-accent text-white rounded-full shadow-sm hover:shadow-md transition-all flex items-center gap-1.5 text-xs",
          className
        )}
      >
        <MessageCircle className="w-4 h-4" />
        <span className="font-bold">觉察</span>
      </motion.button>
            ...
            >
              ...
              <div className="flex justify-between items-center p-6 pb-4 shrink-0 bg-white border-b border-zen-accent/5 sticky top-0 z-10">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xl font-bold font-serif text-zen-ink">念头观察</h3>
                </div>
              ...
              {activeTab === 'record' ? (
                <div className="space-y-6 flex-1 flex flex-col min-h-0">
                  ...
                  <button
                    ...
                    className="w-full py-4 bg-zen-accent text-white rounded-2xl font-bold disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-90 transition-opacity shrink-0 mt-4"
                  >
                    封存念头
                  </button>
                </div>
              ...
              <div className="p-6 pt-4 border-t border-zen-accent/5 bg-white shrink-0">
                <button onClick={() => setIsOpen(false)} className="w-full bg-zen-bg text-zen-accent py-3 rounded-2xl font-bold hover:bg-zen-accent/10 transition-colors">感恩圆满</button>
              </div>
            </motion.div>
          </motion.div>
        )}
