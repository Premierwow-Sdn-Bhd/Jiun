...
export const reportService = {
  ...
  async getReportData(startDate?: Date, endDate?: Date) {
    ...
    if (startDate && endDate) {
      ...
      thoughts = thoughts.filter((t: any) => t.timestamp >= start && t.timestamp < end);
  ...
  async exportCSV(startDate?: Date, endDate?: Date) {
    ...
    thoughts.forEach((t: any) => {
      ...
      const content = `[情绪: ${t.emotions.join(',')}] ${t.event}`.replace(/"/g, '""');
      const result = `${t.action} -> ${t.learning}`.replace(/"/g, '""');
      csvContent += `"念头观察","${date}","${content}","${result}"\n`;
    });
  ...
  async exportHTMLPrint(startDate?: Date, endDate?: Date) {
    ...
    const html = `
      ...
      <head>
        ...
        <title>禅心修行报告</title>
        <style>
          body { font-family: 'Noto Serif SC', 'Songti SC', serif; color: #333; line-height: 1.6; padding: 40px; max-w-4xl; margin: 0 auto; }
      ...
      <body>
        ...
        <h2>念头观察记录 (${thoughts.length} 条)</h2>
        ${thoughts.map((t: any) => `
          <div class="record">
            <div class="record-date">${new Date(t.timestamp).toLocaleString()}</div>
            <div class="record-title">情绪：${t.emotions.join('、')}</div>
